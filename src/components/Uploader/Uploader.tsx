...
export default function Uploader(){
    ...
    const images: (string | File)[] = values?.images || []; 
...
function UploaderItem({ image, id, onRemoveImage }: UploaderItemProps) {
    const ImageContainerOrIcon = () => {
        ...
        return <Box  display={'flex'} width={'100%'} height={'100%'}>
            ...
            <Box className="uploader__item__controls" backgroundColor={'gray.50'} position={'absolute'} gap={3} left={'50%'} top={'50%'} transform={'translate(-50%, -50%)'} borderRadius={'md'} padding={2} shadow={'md'} flexDirection={'row'} display={'none'} >
    ...
    return (
            <Box style={{aspectRatio: '4/3'}} className="uploader__item" position={'relative'}  display={'flex'} justifyContent={'center'} alignItems={'center'} backgroundColor={'gray.50'} shadow={'md'} borderRadius={'md'} _hover={{backgroundColor: 'gray.100'}}>
